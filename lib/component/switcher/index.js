    ...
    next: new Property({
      render (val, props, children, origin, event) {
        ...
        if (data && !(data instanceof Element)) {
          // get rid of smurky tests!
          // if (typeof data === 'string' && data.indexOf('smurky') === 0) {
          //   return
          // }

          // need event in render
    ...
    getPropertyValue (val, event, parent, key, escape) {
      if (val) {
        ...
        if (val instanceof Element) {
          ...
          let delta = axis === 'x'
            ? 1000//this.getNode().clientWidth
            : 500//this.getNode().clientHeight
        ...
        if (switcherNode) {
          ...
          this.each((child, key) => {
            if (child !== elem) {
              ...
              // can be done just with ev- in element no worries that works!
