...
module.exports = new Element({
    ...
    next: new Property({
      render (node, event, switcher) {
        ...
        if (data) {
          ...
          if (!switcher[key]) {
            console.error('dont have make!', key)
            switcher.set({[key]: data})
          } else {
            console.error('have do switch!', key)
  ...
  define: {
    ...
    setTransition (key, event, direction) {
      var elem = this[key]
      console.error('SET TRANSITION', key, elem instanceof Element)
      if (elem instanceof Element) {
        let switcherNode = this.getNode()
        console.error('switcherNode:', switcherNode)
        if (switcherNode) {
          let axis = this.config.axis
          let delta = axis === 'x'
          ...
          this.each(function (child, key) {
            if (child !== elem) {
              ...
              let remove = function () {
                ...
                let sw = child.parent
                if (sw && sw.current.val !== child.key) {
                  child.remove()
                }
              }
          ...
          console.error('SET ZERO', elem.path)
      ...
      if (data) {
        if (data.index) {
          ...
          if (previous && previous.index) {
            if (previous.index.val > data.index.val) {
              direction = -1
            }
          }
        } else if (data.number) {
